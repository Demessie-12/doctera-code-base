...
import { HideProductStatusHook } from "../../Services/apiContributor";
...
function ProductPending() {
  ...
  const verifyProduct = async () => {
    ...
    const updatedData = await HideProductStatusHook(
      productId,
      {
        status: "Pending",
      },
  ...
  return (
    <div className="mt-3 flex items-center justify-center gap-3">
      ...
      <button
        className={`rounded-2xl bg-yellow-500 px-3 py-2 text-center text-xl font-semibold capitalize text-gray-900 md:text-2xl ${isSubmitting ? "cursor-not-allowed bg-red-500 text-white" : ""}`}
